...
## Demo (IUPAC → prompt text with XML)
...
- `opsin-core-*-jar-with-dependencies.jar` — OPSIN jar used by the demo
- `prompts/smiles_iupac_metadata_v13/` — prompt template and semantic sections (default, bridged/fused/spiro ring semantics)
...
- [ ] **Full pipeline to collect the data**
  - [ ] **Simple generation code** — minimal script to go from IUPAC (or SMILES) + metadata to one description (single LLM call).
  - [ ] **Routing example** — example of difficulty-based routing (easy / medium / hard) and model/reasoning settings.
  - [ ] **Whole pipeline** — batch workflow: prepare prompts (e.g. from a molecule list), call batch API to generate descriptions, optional validation and export for the HF dataset.
- [ ] **OPSIN** — README and usage instructions are maintained in [feiyang-cai/opsin_mollangdata](https://github.com/feiyang-cai/opsin_mollangdata); this repo will link to it and document how the JAR is used in our pipeline.
